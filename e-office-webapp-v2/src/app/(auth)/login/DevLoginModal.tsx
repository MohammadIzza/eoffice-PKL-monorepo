  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="w-full gap-1.5 border-dashed text-slate-500 h-8 text-xs">
            <Shield className="w-3.5 h-3.5" />
            Login Cepat
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-3xl h-[520px] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="px-5 py-3 border-b">
          <DialogTitle className="text-base">Developer Quick Login</DialogTitle>
        </DialogHeader>

        <div className="flex flex-1 overflow-hidden">
            {/* Sidebar / Vertical Tab List */}
            <div className="w-40 bg-slate-50 border-r overflow-y-auto py-1.5">
                {ROLES.map((role) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <Badge variant="secondary" className="text-[10px] px-1 h-4 min-w-[16px] justify-center">{count}</Badge>
                        </button>
                    )
                })}
            </div>
            ...
            <div className="flex-1 bg-white flex flex-col">
                <div className="p-3 bg-slate-50/50 border-b">
                    <h3 className="font-semibold text-xs flex items-center gap-1.5">
                        ...
                        Daftar Akun: <span className="text-blue-600">{ROLES.find(r => r.id === activeRole)?.label}</span>
                    </h3>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {DATA_USERS[activeRole]?.map((u, i) => (
                             <button
                                ...
                                disabled={isLoading}
                                className="flex items-center justify-between p-2.5 rounded-lg border border-slate-200 hover:border-blue-300 hover:bg-blue-50 transition-all text-left group bg-white"
                            >
                                <div className="flex flex-col gap-0.5 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                        <span className="font-semibold text-xs text-slate-900 group-hover:text-blue-600 truncate">
                                            {u.name}
                                        </span>
                                        {u.dept && <Badge variant="outline" className="text-[10px] px-1 h-4 text-slate-500 whitespace-nowrap">{u.dept}</Badge>}
                                    </div>
                                    <span className="text-[11px] text-slate-500 truncate font-mono">{u.email}</span>
                ...
                <div className="p-2.5 border-t bg-slate-50 text-center">
                    <p className="text-[11px] text-slate-500">
                        Password default: <code className="bg-slate-200 px-1 rounded text-[10px]">password1234</code>
                    </p>
                </div>
            </div>
        </div>
