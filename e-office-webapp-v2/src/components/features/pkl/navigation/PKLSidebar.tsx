...
export default function PKLSidebar({ mobileOpen = false, onMobileClose }: PKLSidebarProps) {
    ...
    <>
      <div className="flex-1 overflow-y-auto py-4">
        {menuItems.map((section, sectionIndex) => (
          <div key={sectionIndex} className={cn("mb-6", collapsed ? "px-2" : "px-3")}>
            <div
              className={cn(
    ...
    <>
      ...
      <Sheet open={mobileOpen} onOpenChange={(open) => { if (!open) onMobileClose?.(); }}>
        <SheetContent side="left" className="p-0 w-[270px] flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-[rgba(0,0,0,0.08)]">
            ...
            <Button
              ...
              className="h-8 w-8 hover:bg-[rgba(0,0,0,0.04)] rounded-full"
            >
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            {renderMenuItems(false, onMobileClose)}
          </div>
        </SheetContent>
      </Sheet>
    </>
